...
class Agent:
    def __init__(self, agent_id, i_state, g_state, N=50, obstacles=[]):
        ...
        self.jmin = -50
        self.jmax = 50
        self.right_lane_bound = 1000.5
        self.left_lane_bound = -1000.5
        # dt
        self.dt = 0.041
        self.avg_time = []
    ...
    def get_goal_cost(self):
        ...
        for k in range(1,self.N):
            st = self.X[:, k]
            # con = self.U[:, k]
            cost_fn += (st - self.P[self.n_states:]).T @ self.Q @ (st - self.P[self.n_states:]) # [delta_x, delta_y, delta_theta]^T*[Q_x*delta_x, Q_y*delta_y, Q_theta*delta_theta]
        return cost_fn
        # st = self.X[:, self.N]
    ...
    def next_state_constraints(self):
        ...
        for k in range(self.N):
            ...
            st_next_update = st
            st_next_update[0,0] = st[0] + v1*cos(st[2])*self.dt
            st_next_update[1,0] = st[1] + v1*sin(st[2])*self.dt
            st_next_update[2,0] = st[2] + w1*self.dt
